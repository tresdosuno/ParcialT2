...
import static edu.eci.pdsw.samples.entities.TipoIdentificacion.TI;
...
public class ServicesJUnitTest {
    ...
    ServiciosPaciente servicio = ServiciosPacientesFactory.getInstance().getTestingForumServices();
    ...
    @Test
    public void deberiaConsultarPaciente() throws SQLException, ExcepcionServiciosSuscripciones{
        Connection conn=getConnection();
        Statement stmt=conn.createStatement();
        ...
        Assert.assertEquals(ServiciosPacientesFactory.getInstance().getTestingForumServices().consultarPacientesPorId(9876, TI).getId(), 9876);
    }

    @Test
    public void deberiaConsultarPacienteMenor() throws SQLException, ExcepcionServiciosSuscripciones{
        Connection conn=getConnection();
        Statement stmt=conn.createStatement();

        stmt.execute("INSERT INTO `PACIENTES` (`id`, `tipo_id`, `nombre`, `fecha_nacimiento`) VALUES (9877,'TI','Carmen','2020-07-10')");
        stmt.execute("INSERT INTO `CONSULTAS` (`idCONSULTAS`, `fecha_y_hora`, `resumen`, `PACIENTES_id`, `PACIENTES_tipo_id`) VALUES (1262219,'2001-01-01 00:00:00','varicela',9877,'TI')");

        conn.commit();
        conn.close();

        Assert.assertEquals(ServiciosPacientesFactory.getInstance().getTestingForumServices().consultarMenoresConEnfermedadContagiosa().get(0).getId(), 9877);
    }
